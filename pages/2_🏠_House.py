...
fig = px.pie(df.groupby("Quận").count().reset_index(),values="Địa chỉ",  names="Quận", title='Số lượng nhà ở của các quận, huyện')
table_1.dataframe(df.groupby("Quận").count()[["Loại hình nhà ở"]], use_container_width=True)
chart_1.plotly_chart(fig)



st.subheader("2. Biểu đồ thể hiện tỉ lệ loại hình nhà ở (mặt tiền, hẻm) ở các quận, huyện")
district_df = df[["Quận","Loại hình nhà ở"]]
table_2, chart_2 = st.columns(2)

districts = table_2.multiselect(label="Chọn quận", options=district_df["Quận"].unique())
is_select_all_districts = table_2.checkbox(label="Chọn tất cả", value=True, key="is_select_all_districts")
types = chart_2.multiselect(label="Chọn loại hình nhà ở", options=district_df["Loại hình nhà ở"].dropna().unique())
is_select_all_types = chart_2.checkbox(label="Chọn tất cả", value=True, key="is_select_all_types")

if is_select_all_districts:
    districts = district_df["Quận"].unique()

if is_select_all_types:
    types = district_df["Loại hình nhà ở"].unique()

filterd_df = district_df.loc[district_df["Quận"].isin(districts) & district_df["Loại hình nhà ở"].isin(types)]   
grouped_df = district_df.groupby(["Quận","Loại hình nhà ở"])["Loại hình nhà ở"].count().unstack()
...
fig = px.histogram(filterd_df, x=filterd_df["Quận"], color=filterd_df["Loại hình nhà ở"], barmode='group',
             height=400) 
...
district_df = df[["Quận","Loại hình nhà ở"]]
selected_district = st.selectbox(label="Chọn quận", options=district_df["Quận"].unique())
fig = px.pie(district_df.loc[district_df["Quận"] == selected_district].groupby("Loại hình nhà ở").count().reset_index(),values="Quận",  names="Loại hình nhà ở", title=f'Loại hình nhà ở tại {selected_district}')
st.plotly_chart(fig)



st.subheader("4. Quận nào có tỉ lệ nhà nào cao nhất?")
THRESHOLD = 10
filterd_df = district_df.loc[district_df["Quận"].isin(districts) & district_df["Loại hình nhà ở"].isin(types)]   
grouped_df = district_df.groupby(["Quận","Loại hình nhà ở"])["Loại hình nhà ở"].count().unstack()
grouped_df = grouped_df.fillna(0).astype(int)
...
fig = px.pie(the_best_district, values=the_best_district.columns[0], names=the_best_district.index, title=f'Quận có tỉ lệ nhà hẻm cao nhất là {the_best_district.columns[0]}')
...
the_best_district = grouped_df.head(1).drop(["Tổng"], axis=1).transpose()
fig = px.pie(the_best_district, values=the_best_district.columns[0], names=the_best_district.index, title=f'Quận có tỉ lệ nhà mặt phố, mặt tiền cao nhất là {the_best_district.columns[0]}')
best_chart.plotly_chart(fig, use_container_width=True)
...
the_best_district = grouped_df.head(1).drop(["Tổng"], axis=1).transpose()
fig = px.pie(the_best_district, values=the_best_district.columns[0], names=the_best_district.index, title=f'Quận có tỉ lệ biệt thự cao nhất là {the_best_district.columns[0]}')
best_chart.plotly_chart(fig, use_container_width=True)
